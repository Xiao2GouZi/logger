...
>>[noEmitHelpers](https://github.com/Xiao2GouZi/logger/blob/ts/material/ts/compilerOptions.md#noEmitHelpers) | boolen | false | 不在输出文件中生成用户自定义的帮助函数代码
...
>>[sourceRoot ](https://github.com/Xiao2GouZi/logger/blob/ts/material/ts/compilerOptions.md#sourceRoot ) | string |  | 指定TypeScript源文件的路径，以便调试器定位。当TypeScript文件的位置是在运行时指定时使用此标记。路径信息会被加到 sourceMap里
...
>>[forceConsistentCasingInFileNames](https://github.com/Xiao2GouZi/logger/blob/ts/material/ts/compilerOptions.md#forceConsistentCasingInFileNames) | boolen | false | 禁止对同一个文件的不一致的引用。
...
>>[out](https://github.com/Xiao2GouZi/logger/blob/ts/material/ts/compilerOptions.md#out) | boolen | false | 弃用
